...
def process_embed_order(langid):
    ...
    sav_remain_ents = True
    word4notfound = False
    BASELINE = False
    dim = 300
    overwrite = False

    # read sav_remain_ents
    if sav_remain_ents is False:
        reepro2ree = {}
        with open(f"{out_dir}/ree2reepro_{id2lang[langid]}", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip().split()
                reepro2ree[line[1]] = line[0]
    ...
    if BASELINE is True or word4notfound is True:
    ...
    word2id_wiki2vec = {}
    embeddings_wiki2vec = []
    with open(f"../data/wiki2vec/{id2lang[langid]}/{id2lang[langid]}wiki_20180420_300d.txt", "r", encoding="utf-8") as f: #{id2lang[langid]}wiki_20180420_300d.txt" # {id2lang[langid]}-fasttext-subsample.vec
        for index, line in enumerate(f):
            ...
            elif sav_remain_ents is False and word in reepro2ree:
                word_ori = reepro2ree[word]
                if word_ori in word2id: # found duplicate
                    continue
                found += 1
                ents.add(word_ori)
                word2id[word_ori] = len(word2id)
                vectors.append(vect)
    ...
    for e in remain_ents:
        ...
        if sav_remain_ents is True:
            e_pro = e.split("_(")[0]
            if e_pro in word2id_wiki2vec:
    ...
    #save remain_ents2file
    if sav_remain_ents is True:
        with open(f"{out_dir}/ree2reepro_{id2lang[langid]}", "w") as f:
            for k, v in ree2reepro.items():
                f.write(f"{k}\t{v}\n")
        return
    ...
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    fout_name = f"{out_dir}/{id2lang[langid]}wiki_300d_pro_4notfound.txt"
